...
$rootDir = -join($drive, "\")
Get-ChildItem $rootDir -recurse | get-acl | select * | Export-Csv -Path $PSScriptRoot"\"$outputDir"\04_01-"$outputDir"-File_Permissions.csv" 
...
Get-WmiObject Win32_service -filter "StartMode = 'auto' " | Export-Csv -Path $PSScriptRoot"\"$outputDir"\04_05-"$outputDir"-Autostart.csv"